...
#brew install wget ninja glm android

mkdir -p android-dependencies-src && cd android-dependencies-src 
...
(
    ...
    -DCMAKE_FIND_ROOT_PATH=/opt/android-ndk/toolchains/llvm/prebuilt/linux-x86_64/sysroot \
		-DCMAKE_INSTALL_PREFIX=$HOME/deps-android-native \
		-G Ninja
	ninja install
)
...
tar -xzf 2.87.tar.gz && (
	...
	mkdir -p build-android && cd build-android
	cmake .. \
		-DCMAKE_TOOLCHAIN_FILE="../../toolchains/generic/Android-wasm.cmake" \
		-DCMAKE_BUILD_TYPE=Release \
...
# Crosscompile zstd. Version 1.5.1+ doesn't compile with this Android
# version, saying that
#   huf_decompress_amd64.S: Input file has an unknown suffix, don't know what to do with it!
# Newer Androids work fine, 1.5.0 doesn't have this file yet so it works.
...
tar -xzf v$ZSTD_VERSION.tar.gz && (
	...
	cmake ../build/cmake \
		-DCMAKE_TOOLCHAIN_FILE="../../magnum-integration/toolchains/generic/Android-wasm.cmake" \
